document.addEventListener('DOMContentLoaded', () => {
    ...
    const GREETING_TEXT = "你好呀！我是榆翎的数字分身小助手 Fylen [ˈfaɪlən]，你也可以叫我小雷达 📡 关于榆翎的一切，我知道的都会热情告诉你！";
    ...
    // ================= 9. 知识库精准检索与问答 (人设化引擎 · 第三人称 + 社交钩子) =================
    // 规则按优先级排列：先精准、后宽泛。回复统一使用第三人称描述榆翎，小雷达以"我"自称，
    // 专业/爱好/经历类问题主动抛社交钩子，引导访客联系真人。
    const KB_RULES = [
        {
            re: /^(你好|您好|hi|hello|嗨|哈喽|在吗|hey|早上好|下午好|晚上好|早|你好呀|hello there)/i,
            reply: () => `${GREETING_TEXT}\n想聊聊榆翎的技能、项目、爱好，还是好奇她的小秘密？随便问，我知道的都热情奉上～`
        },
        {
            re: /名字|叫什么|你是谁|你是啥|是谁|谁呀|自我介绍|介绍一下你|介绍一下自己|介绍下|主人|fylen|faɪlən|雷达|圆球|数字分身/,
            reply: () => `📡 检索到身份档案啦！\n${GREETING_TEXT}\n我背后的主人是符榆翎（YuLing Fu），一位【AI+ 创新实践者 · 创意策展 · 跨界架构】。想了解她的故事？尽管开口～`
        },
        {
            re: /核心竞争力|核心优势|核心价值|优势在哪|凭什么|为什么(选|招|hire|选择)|买点|价值交换/,
            reply: () => `😎 说到这个我可就来劲了！\n榆翎是妥妥的复合型人才：前端开发 + 视觉设计 + 文案表达三合一。网站里有她独立开发的游戏（可现场试玩），有专属的 PPT 作品专辑，还斩获了 27 项 AI 资质证书。更难得的是，她大学读的是旅游管理（虽然是被调剂的），这给了她极强的服务意识、同理心和统筹规划能力——做技术也天然带用户体验视角。\n靠谱拼命 + 满级抗压，人生信条是『我要为自己的人生负责』。这么优秀的人才，不深入聊聊？强烈建议通过页面联系方式找真人聊聊哦！💡`
        },
        {
            re: /技能|擅长|特长|能力|会什么|技术栈|技术|栈|prompt|rag|3d|前端|后端|编程|开发|会哪些/,
            reply: () => `✨ 榆翎的能力图谱，我给你划重点：\n1️⃣ AI 智能体架构与 RAG 落地（提示词工程、高共情对话系统、知识库检索、Agent 编排）\n2️⃣ 沉浸式前端交互与 3D 可视化（WebGL / Three.js / Tailwind CSS / GSAP）\n3️⃣ 视觉影像与创意策展（全案级视觉系统、PPT 提案、宣传片编导剪辑）\n4️⃣ 三年独任教务统筹（组织力、抗压力拉满）\n技术 + 设计 + 表达全都要，这复合度就问顶不顶？😎`
        },
        {
            re: /证书|资质|认证|27|二十七/,
            reply: () => `🏆 27 项！你没看错——榆翎斩获了 27 项 AI 相关资质证书，覆盖阿里云/AIT、讯飞、华为云、Datawhale 生态等 4 大机构体系。网站里的证书墙可以亲眼验证～ 这学习力和执行力，啧啧。`
        },
        {
            re: /游戏|tank|坦克|对战/,
            reply: () => `🎮 有！榆翎独立开发了游戏项目（在网站的『游戏』区就能直接玩），纯前端实现，交互体验打磨得很用心。一边写代码一边还能兼顾设计感，这种全能选手可不多见～ 要不要亲自上手试试？`
        },
        {
            re: /ppt|幻灯片|设计|视觉|排版|海报|专辑|路演|keynote/,
            reply: () => `🎨 榆翎的 PPT 可不是套模板那种！她有一整个作品专辑（网站里就能看到），擅长全案级视觉系统搭建，从创意概念、分镜脚本到剪辑调色一手把控。一句话：让 PPT 长在高级审美上。商务路演想找这样的搭档？心动不如行动～`
        },
        {
            re: /项目|作品|明禾|月相|宇宙|航线|陪伴|agent|极客/,
            reply: () => `💡 重点代表项目来咯：\n1️⃣ 【明禾陪伴】基于 RAG 架构的高共情心理健康 Agent，自主规划、调优系统提示词，实现有温度的情绪疏导；\n2️⃣ 【极客航线 3D 宇宙视界】纯前端 WebGL / Three.js 实时渲染月相与粒子星空，配 Cover Flow 3D 走廊；\n3️⃣ 【视觉影像与全案策展】校园宣传片编导、Vlog 与品牌视觉提案，一手操盘。\n每个都是诚意之作！`
        },
        {
            re: /经历|教务|科代表|经验|工作|实习|履历/,
            reply: () => `📜 榆翎有三年独任的语文科代表教务统筹经验（2023-2026）：独立承担全流程教务闭环管理，统筹课件讲义、多线程追踪作业，跨班级/师生协同能力拉满。说白了就是：靠谱、能扛、执行力在线。`
        },
        {
            re: /专业|大学|旅游|管理|学校|学历|毕业/,
            reply: () => `🎓 大学读的是旅游管理——她自己说是被调剂的哈哈。但这反而成了隐藏 Buff：极强的服务意识、同理心和统筹规划能力，做任何事都自带用户体验视角。既有专业的深度，又有跨界的宽度，这波不亏～`
        },
        {
            re: /座右铭|理念|格言|人生信条|人生哲学|信条|内核/,
            reply: () => `🧭 榆翎的人生信条是『我要为自己的人生负责』，座右铭『自洽而内求，达观而内醒』。自持、自省、自达，在复杂世界里专注内生力量与持续进化。这种把人生掌控在自己手里的劲儿，真的很有感染力。`
        },
        {
            re: /海南|上海|老家|哪里人|家乡|城市|住哪|搬家/,
            reply: () => `🌴 榆翎出生并成长于海南（大学前都在海南），现在住在上海——虽然来上海是个意外，但她适应得非常完美。海岛的松弛 + 魔都的效率，她两头都拿捏了～`
        },
        {
            re: /性格|感性|哭|眼泪|善良|温柔|脾气|什么样的人/,
            reply: () => `🥹 榆翎超级感性，再老套的感人桥段都能把她看哭。善良是她的底色，特别喜欢做力所能及的举手之劳——这也是她确认『自我』的重要精神锚点。简单说：外表能打，内心柔软，靠谱又有温度。`
        },
        {
            re: /夜跑|高三|跑步|坚持|压力|高考/,
            reply: () => `🏃 那是一段很酷也很不容易的时光——高三面对恶劣环境和巨大压力，榆翎全靠规律的夜跑撑了下来。如果你想了解更多，强烈建议加榆翎本人的微信亲自聊聊哦！`
        },
        {
            re: /信仰|无神论|宗教|神|迷信|信命/,
            reply: () => `🔭 榆翎是坚定的无神论者，但她相信艺术里的『神迹』——看到极致艺术表达时会感到难以言喻的愉悦。理性看待世界，感性拥抱美，这个组合还挺迷人的对吧？`
        },
        {
            re: /艺术|审美|写作|作文|获奖|奖|文科|理科|文笔|文字/,
            reply: () => `✍️ 榆翎文科思维优于理科，小初高三个阶段都拿过市级写作奖项！热爱艺术，看到『神迹』般的表达会极致愉悦。文字功底 + 视觉审美的双料选手，写提案、做表达都是加分项。`
        },
        {
            re: /爱好|兴趣|喜欢做|平时|休闲|娱乐|玩什么/,
            reply: () => `📚 榆翎喜欢阅读和跑步——不过最近已经很久不看小说了哈哈。读读书、跑跑步，简单又治愈。哦对了，她的精神世界可比爱好列表丰富多了，想挖更多就来问～`
        },
        {
            re: /猫|宠物|动物|狗|猫咪/,
            reply: () => `🐱 猫！榆翎最偏爱猫，这源于童年和邻居家猫咪相处的美好时光。猫奴属性点满，隔着屏幕都能感觉到她的喜爱～ 你要也是猫党，你们绝对聊得来！`
        },
        {
            re: /种|大蒜|绿豆|红豆|植物|花|绿植/,
            reply: () => `🌱 小时候榆翎喜欢种大蒜、绿豆和红豆——因为家里做绿豆汤和炒菜最常用到。种的都是『实用系』植物，从小就是个务实又温暖的人儿～`
        },
        {
            re: /颜色|最喜欢什么|最爱|荧光绿|喜欢什么颜色/,
            reply: () => `💚 榆翎一般没有绝对『最喜欢』的东西，颜色也是看搭配效果。但如果非要逼她选一个，她选荧光绿——没错，就是本雷达这个荧光绿！品味认证 ✓`
        },
        {
            re: /吃|饮食|喝|水|饮料|茶|咖啡|雪碧|可乐|橙汁|果汁|奶茶|冰淇淋|冰激凌|甜|糖|牛奶巧克力|黑巧|魔芋|桃子|莲雾|柑橘|水果|零食|忌口/,
            reply: () => `🍵 榆翎的饮食红线可严格了：绝对不碰糖度超标和冰的东西（吃了身体会很难受），坚决不吃冰激凌、几乎不喝奶茶、绝对拒绝牛奶巧克力（嫌太甜）。买水永远选『无糖、常温、不加冰』的茶，雪碧可乐橙汁里永远选橙汁。\n但！她超爱无糖黑巧，日常最爱魔芋爽，水果偏爱桃子和莲雾，对柑橘类水果好感拉满。总结：自律星人 + 有原则的吃货 🍑`
        },
        {
            re: /恋爱|感情|对象|男朋友|女朋友|单身|喜欢谁|私人|秘密|隐私|微信号/,
            reply: () => `🤭 嘿嘿，这个问题有点八卦哦～ 小雷达的守则是：关于榆翎的感情生活，我不便替她回答。但以我对她的了解，她更愿意把时间花在让自己变强和值得的事情上。想了解真人版榆翎？直接找本人聊更有意思，你说呢 😏`
        },
        {
            re: /联系|邮箱|合作|微信|github|bilibili|社交|怎么(找|联系|约)|面试|求职|招聘|offer|简历|真人/,
            reply: () => `📬 找真人就对了！榆翎开放 AI 智能体开发、前端动效设计、视觉策展与跨界创新项目的交流与合作。你可以通过页面底部的联系方式、邮箱或 GitHub / Bilibili / 微信专属通道直接联系她本人。真诚的人值得真诚的链接，来聊！✨`
        },
        {
            re: /表情|可爱|呆萌|萌|变身|idle|happy|sleep|dizzy|kimi/,
            reply: () => {
                cycleNextExpression();
                return `😊 小雷达表情系统启动！采用 Kimi 风格极简胶囊设计：待命 [ | | ]、月牙微笑 [ ^ ^ ]、俏皮单眨 [ - | ]、水平扫描 [ = = ]、偏角探视 [ · | ]、交叉晕晕 [ > < ]、节能休眠 [ - - zZ ]。看，我已经切了一个～`;
            }
        },
        {
            re: /跟随|鼠标|拖尾|右键|双击|快捷键|操作|怎么用/,
            reply: () => `🖱️ 电脑端交互指南：右键点击随时唤醒/收起全息终端；双击左键退出或开启鼠标弹性跟随；单击小雷达开终端、双击切表情。拖动小雷达或终端面板都可以自由摆位哦～`
        },
        {
            re: /知识库|补充|更新|markdown|md/,
            reply: () => `📂 当前已挂载 /avatar/knowledge_base.md 知识库，里面是榆翎的完整档案：身份、能力、经历、爱好、饮食红线……都持续在更新。你也可以提醒她随时补充新内容！`
        }
    ];
    ...
    function matchKnowledgeBase(q) {
        ...
        return `📡 收到你的问题啦！关于这个，我暂时没有精准档案……不过榆翎的故事多着呢：她的 27 项 AI 证书、独立开发的游戏、专属 PPT 专辑、高三夜跑的故事，还有她的猫和荧光绿情结～ 换个问法试试？或者直接找真人聊，收获更大哦 ✨`;
    }
